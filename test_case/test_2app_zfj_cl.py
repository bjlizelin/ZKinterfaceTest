...
class test_zfj_post(MyTest):         #把这个接口封装一个类，下面的方法是具体的测试用例  
    ...
    def test_jcjs_down(self): 
        try:   
                '''''测试用例1：江城集市处下派_执法局处理'''   #这个描述接口用例名称   
            ...
            #                 #取首行案卷pfryid和tsid
                ...
                r1 = requests.post(url = self.url1,data = self.data1,headers =self.headers1)     
                print("执法局处理江城集市上报成功"+r1.text)
                logging.info("执法局处理江城集市上报成功"+r1.text)   
        except Exception, e:
              print str(e)
              return False
              print("执法局处理江城集市上报失败"+r1.text)
              logging.info("执法局处理江城集市上报失败"+r1.text)         
