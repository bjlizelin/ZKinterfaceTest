...
class test_zfj_post(MyTest):         #把这个接口封装一个类，下面的方法是具体的测试用例  
    ...
    def test_jcjs_down(self): 
        '''''测试用例1：非市场办执法局上报_执法局市场办派发_江城集市处理'''   #这个描述接口用例名称   
        ...
        r = requests.post(url = self.url,data = self.data,headers = self.headers,timeout=30) 
        a=r.text 
        b=json.loads(a)
        c=str(b['status'])
        if c=="1":
            print("非市场办执法局上报成功"+self.r.text)
            logging.info("非市场办执法局上报成功"+self.r.text)
            return True
        else: 
            logging.info("非市场办执法局上报失败"+self.r.text)
            print("非市场办执法局上报失败"+self.r.text) 
            return False 
        ...
        self_data = urllib2.urlopen(url,timeout=30)
        ...
        r1 = requests.post(url = url1,data = data1,headers =headers1,timeout=30)      
        a1=r.text 
        b1=json.loads(a1)
        c1=str(b1['status'])
        if c1=="1":
...
#         logging.info(self2)
        ...
        r1 = requests.post(url = self.url2,data = self.data2,headers =self.headers2,timeout=30)     
        a2=r1.text 
        b2=json.loads(a2)
        c2=str(b2['status'])
        if c2=="1":
